...
export default class Analyser {
  ...
  constructor(i18n, keywordInput, scoreInput) {
    ...
    this.unsubscribeToStore = this.store.subscribe(() => {
      ...
      console.log("Store updated, will assess paper: ", state.paper !== previousPaper || state.selectedKeyword !== previousKeyword);
      ...
      if (state.paper !== previousPaper || state.selectedKeyword !== previousKeyword) {
        ...
        this.store.dispatch(
          assessPaper(previousKeyword, contentScore, contentAssessments)
        );

        console.log(contentScore, contentAssessments);
      }
    });
  }
