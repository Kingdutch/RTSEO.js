 ...
 * @param {object} t The Drupal.t function.
 * @returns {{score: number, text}} resultobject with score and text.
 */
const calculatePassiveVoiceResult = function( passiveVoice, t ) {
  let score;
  let percentage = 0;
  ...
  if ( score >= 7 ) {
    return {
      score: score,
      hasMarks: hasMarks,
      text: t("Passive Voice: You're using enough active voice. That's great!"),
    };
  }
  return {
    score: score,
 ...
 */
const passiveVoiceAssessment = function( paper, researcher, t ) {
  const passiveVoice = researcher.getResearch( "passiveVoice" );

  const passiveVoiceResult = calculatePassiveVoiceResult( passiveVoice, t );
...
export default {
  ...assessments.PassiveVoiceAssessment,
